...
psa_status_t TokenSet::getCurrentB64Token(char *token_out) const {
    // timestamp
    size_t olen;
    if (mbedtls_base64_encode(reinterpret_cast<unsigned char *>(token_out), TIMESTAMP_B64LEN + 1, &olen,
                              this->timestamp,
                              TIMESTAMP_LEN) != 0 || olen != TIMESTAMP_B64LEN)
...
psa_status_t TokenSet::sealCli2Serv(uint8_t *in_out, const size_t in_out_len) const {
    ...
    return psa_aead_encrypt(
        ...
        in_out, in_out_len - this->getTagLen(), in_out, in_out_len, &ciphertext_len
    );
}
