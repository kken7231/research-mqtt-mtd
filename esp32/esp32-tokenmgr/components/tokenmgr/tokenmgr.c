...
static void mqtt_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
	...
	if (event_id == MQTT_EVENT_CONNECTED) {
		ESP_LOGI(TAG, "MQTT (%s) connected", client_type_label);
		xEventGroupSetBits(client_group, MQTT_CONNECTED_BIT);
	} else if (event_id != MQTT_EVENT_BEFORE_CONNECT) {
		ESP_LOGE(TAG, "MQTT (%s) event other than connected: %ld", client_type_label, event_id);
...
static esp_err_t mqtt_init(void) {
	...
	EventBits_t bits = xEventGroupWaitBits(mqtt_plain_event_group,
										   ...
										   pdFALSE, pdFALSE, portMAX_DELAY);
	ESP_ERROR_CHECK(esp_mqtt_client_unregister_event(plain_mqtt_client, ESP_EVENT_ANY_ID, mqtt_event_handler));
	...
	bits = xEventGroupWaitBits(mqtt_tls_event_group,
							   MQTT_CONNECTED_BIT | MQTT_FAIL_BIT,
							   pdFALSE, pdFALSE, portMAX_DELAY);
	ESP_ERROR_CHECK(esp_mqtt_client_unregister_event(tls_mqtt_client, ESP_EVENT_ANY_ID, mqtt_event_handler));
	if (bits & MQTT_CONNECTED_BIT)
...
esp_err_t mqtt_publish_qos0(mqtt_client_type_t client_type, const char *topic, const char *data, int len_data) {
	...
	const char *mqtt_client_label = (client_type == MQTT_CLIENT_PLAIN) ? "plain" : "tls";
	const esp_mqtt_client_handle_t mqtt_client = (client_type == MQTT_CLIENT_PLAIN) ? plain_mqtt_client : tls_mqtt_client;
	if (esp_mqtt_client_publish(mqtt_client, topic, data, len_data, 0, 0) != 0) {
		ESP_LOGI(TAG, "MQTT Publish (%s) failed", mqtt_client_label);
		ret = ESP_FAIL;
	} else {
