...
var (
	TOKENS                     = make([][]byte, 0)
	TOPIC_FILTERS              = make([][]byte, 0)
	OUTPUT_ONLY_TOKEN          = false
	OUTPUT_B64                 = false
	NUM_OF_FETCH_TOKENS uint16 = 1
)

func getFilePath(host string, port uint16, topicFilter string) string {
	return fmt.Sprintf("%s/subscribe/%s:%d:%s", common.CLIENT_OUTPUT_DIRECTORY, host, port, common.BytesToB64EncodedString(unsafe.Slice(unsafe.StringData(topicFilter), len(topicFilter))))
}
...
func isValidTopicFilter(topicFilter string) bool {
	...
	for i, level := range levels {
		// Check for invalid characters in each level
		...
		// Invalid if '+' or '#' is used incorrectly within a level
...
func connectTLS(host string, port uint16, topicFilter string) ([]byte, error) {
	if len(topicFilter) > 0xFFFF {
		panic("Topic filter too long")
	}
	defer func() {
		if r := recover(); r != nil {
	...
	var topicFilterLen uint16 = uint16(len(topicFilter))
	data := make([]byte, 0, 4+topicFilterLen)

	data = binary.BigEndian.AppendUint16(data, NUM_OF_FETCH_TOKENS)
	data[0] |= common.FETCH_TOKEN_FOR_SUB
	data = binary.BigEndian.AppendUint16(data, topicFilterLen)
	data = append(data, unsafe.Slice(unsafe.StringData(topicFilter), int(topicFilterLen))...)
	conn.Write(data)
	...
	err = storeTokens(host, port, topicFilter, response[common.TOKEN_LEN:])
	return response[:common.TOKEN_LEN], err
}

func storeTokens(host string, port uint16, topicFilter string, tokensToBeSaved []byte) error {
	if len(tokensToBeSaved) == 0 {
		return nil
	}
	if len(tokensToBeSaved) < common.TOKEN_LEN {
		panic("No data as randomized topic filters")
	}
	return os.WriteFile(getFilePath(host, port, topicFilter), tokensToBeSaved, os.ModePerm)
}
...
func getToken(host string, port uint16, topicFilter string) ([]byte, error) {
	filePath := getFilePath(host, port, topicFilter)
	if _, err := os.Stat(filePath); err == nil {
		return popToken(filePath)
	}
	return connectTLS(host, port, topicFilter)
}
...
func main() {
	...
	topic := flag.String("topic", "", "MQTT topic filter")
	flag.Parse()
	...
	if !isValidHostname(*host) {
	...
	} else if !isValidTopicFilter(*topic) {
		fmt.Println("Invalid topic filter. It must be an ASCII string with printable characters.")
		...
		// Proceed with connection and topic filter fetching
